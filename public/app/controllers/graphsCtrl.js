(function () {
    ...
    function projectManDaysGraph($scope, $filter, allocationService, projectService) {
        ...
        projectService.getProject().then(function (project) {
            allocationService.getAllAllocationByYear('2017', '2019', 'ALL').then(function (allocation) {
                drawTotalManDaysGraph($filter, project.data, allocation.data);
            }).catch(function (err) {
                console.log(err);
            });
    ...
    function drawTotalManDaysGraph($filter, projectList, allocationList) {
        var records = [];
        var months = ["Feb-18", "Mar-18", "Apr-18", "May-18", "Jun-18"];

        angular.forEach(projectList, function (project) {
            var monthWise = new Array(months.length);
            ...
            angular.forEach(filterAllocation, function (alloc) {
                angular.forEach(alloc.allocation, function (data) {
                    if (months.indexOf(data.month) >= 0) { // check if months equal to the predefined month array(user selected)
                        var indx = months.indexOf(data.month);
            ...
            records.push({ label: project.projectname, backgroundColor: getRandomColor(), data: monthWise });
        });
