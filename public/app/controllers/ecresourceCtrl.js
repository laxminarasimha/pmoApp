(function () {
    ...
    angular.module('pmoApp').controller('ecresourceCtrl', Controller);
    ...
    function Controller($scope, $rootScope, resourceService, designationService, DTOptionsBuilder, DTColumnBuilder, $compile,$filter, skillSetService, locationService, roleService, $window) {
        ...
        $scope.delete = function (event) {
            ...
            resourceService.deleteEbResource($scope.deletedID).then(function (res) {
                if (res.data == "deleted") {
                    getResourceData(resourceService, $scope);
                    app.loading = false;
        ...
        $scope.childInfo = function (cen, yearSelect, event) {
            ...
            scope.records = $filter('filter')($scope.ebResource, {CEN: cen});
            ...
            if (row.child.isShown()) {
                ...
                row.child($compile('<div ebresource-child-directive class="clearfix"></div>')(scope)).show();
                tr.addClass('shown');
            }
        }
       ...
       function getResourceData(resourceService, $scope) {
        resourceService.getEBResources().then(function (res) {
            $scope.ebResource = res.data;
        }).catch(function (err) {
            console.log(err);
        });
    }
